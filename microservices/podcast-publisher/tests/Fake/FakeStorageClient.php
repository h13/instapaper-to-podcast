...
final class FakeStorageObject extends StorageObject
{
    ...
    public function signedUrl($expires, array $options = []): string
    {
        ...
        $baseUrl = 'https://storage.googleapis.com/' . $this->bucket->getName() . '/' . $this->name;
        $signature = substr(md5($this->name . (string)$expires), 0, 16);
        
        return $baseUrl . '?X-Goog-Signature=' . $signature;
    }
}